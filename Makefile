...
help:
	...
	@echo -e "  $(YELLOW)make go-tidy$(RESET)            - go mod tidy для всех модулей"
	@echo -e "  $(YELLOW)make go-fmt$(RESET)             - go fmt для всех модулей"
	@echo -e "  $(YELLOW)make go-vet$(RESET)             - go vet для всех модулей"
	@echo -e "  $(YELLOW)make go-test$(RESET)            - go test для всех модулей"
	@echo -e "  $(YELLOW)make go-build$(RESET)           - go build для всех backend сервисов"
	@echo -e "  $(YELLOW)make go-clean$(RESET)           - Очистить Go кэш и бинарники"
	@echo -e ""
	@echo -e "$(GREEN)🔑 Тестовые доступы:$(RESET)"
	@echo -e "  $(CYAN)Customer:$(RESET)   $(YELLOW)$(TEST_CUSTOMER_LOGIN)$(RESET) / $(YELLOW)$(TEST_CUSTOMER_PASS)$(RESET)"
	@echo -e "  $(CYAN)Courier:$(RESET)    $(YELLOW)$(TEST_COURIER_LOGIN)$(RESET) / $(YELLOW)$(TEST_COURIER_PASS)$(RESET)"
	@echo -e "  $(CYAN)Restaurant:$(RESET) $(YELLOW)$(TEST_RESTAURANT_LOGIN)$(RESET) / $(YELLOW)$(TEST_RESTAURANT_PASS)$(RESET)"
	@echo -e ""
